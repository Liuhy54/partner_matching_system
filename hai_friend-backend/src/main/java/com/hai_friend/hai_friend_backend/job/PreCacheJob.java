package com.hai_friend.hai_friend_backend.job;
        ...
        try {
            ...
            if (lock.tryLock(0, -1, TimeUnit.MILLISECONDS)) {
                ...
                for (Long userId : mainUserList) {
                    ...
                    String redisKey = String.format("haiFriend:user:recommend:%s" ,userId);
                    ValueOperations<String, Object> valueOperations = redisTemplate.opsForValue();// 写入缓存
                    // 写缓存
                    try {
            ...
            log.error("redis lock doCacheRecommendUser error", e);
        } finally {
            //只能释放自己的锁
